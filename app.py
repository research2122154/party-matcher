...
def generate_full_schedule(people_list, num_tables, past_met_pairs=None, total_rounds=3, max_attempts=1000, progress_bar=None, status_text=None):
    ...
    for attempt in range(max_attempts): 
        ...
        current_met_pairs = set() # 당일 중복 만남 기록용
                ...
                for _ in range(t_size):
                    ...
                    for p in unseated:
                        ...
                        for seated in round_tables[t_idx]:
                            # [핵심 변경] 이성(남-여)일 경우에만 중복 만남 패널티 적용
                            if p['성별'] != seated['성별']:
                                pair = tuple(sorted([p['고유ID'], seated['고유ID']]))
                                if pair in current_met_pairs:
                                    p_penalty += 100000  # 1순위: 당일 이성 중복 만남 금지
                                elif pair in past_met_pairs_set:
                                    p_penalty += 50000   # 3순위: 과거 이성 중복 만남 금지
                                    
                            # 6순위: 동일 학교 & 동일 학과 충돌 방지 (20,000점)
                        ...
                        # 2순위: 성비 유지 (100,000점 - 1순위와 동급의 절대 방어)
                        ...
                        # 4순위: N개 대학 쏠림 분산 및 고갈 방지 (30,000점)
...
if uploaded_file is not None:
    ...
    for col in df.columns:
        if ('학교' in col or '대학' in col) and '재학중인대학' not in rename_dict.values() and col not in rename_dict:
             rename_dict[col] = '재학중인대학'
    ...
    if not {'이름', '성별', '재학중인대학'}.issubset(df.columns):
        ...
        if 'selected_df' in st.session_state:
            ...
            if len(wait_df) > 0:
                ...
                # [수정 2] 대기자 명단 테이블 바로 아래에 다운로드 버튼 배치
            ...
            if st.button("🚀 2단계: 선발된 참가자로 전체 라운드(1~3) 배치 구동!", use_container_width=True):
                ...
                for r_idx, round_tables in enumerate(all_rounds_data):
                    for t_idx, table in enumerate(round_tables):
                        ...
                        # 품질 검증 리포트에서도 '이성' 간 중복 만남만 색출하도록 수정
                        for i in range(len(table)):
                            for j in range(i+1, len(table)):
                ...
                col_r1.metric("🚨 이성 중복 만남 횟수", f"{dup_meets}건")
